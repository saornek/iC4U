...
df_labels = pd.read_csv("/home/ic4u/zed/codes/ic4u_GroceryAssistant/cv_viewer/groceryLabelsResponse.csv")
    ...
    for obj in objects.object_list:
        if render_object(obj, is_tracking_on):
            ...
            if (isClassFound == True):
                ...
                objectAsinID = df_labels.loc[df_labels.classid == obj.raw_label, 'asin'].values[0]
                print(objectAsinID)
                text = str(objectCustomLabel)
                exporter.values(str(objectAsinID))
            else:
