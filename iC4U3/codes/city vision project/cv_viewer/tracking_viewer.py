...
import pandas as pd
import cv_viewer.exporter as exporter

df_labels = pd.read_csv("/home/ic4u/zed/codes/ic4u_TrafficLight/cv_viewer/ic4uWeightV2.csv")
    ...
    for obj in objects.object_list:
        if render_object(obj, is_tracking_on):
            ...
            if (isClassFound == True):
                ...
                objectResponse = df_labels.loc[df_labels.classid == obj.raw_label, 'response'].values[0]
                if (obj.raw_label == 0):
                    #print(obj.bounding_box_2d, type(obj.bounding_box_2d))
                    crop_img = objectCrop[int(top_left_corner[1]):int(top_left_corner[1] + roi_width), int(top_left_corner[0]):int(top_left_corner[0] + roi_height)]
                    cv2.imwrite("crop.jpg", crop_img)
                    text = (str(objectCustomLabel) + ", " + exporter.detectColor("crop.jpg"))
                else:
                    text = str(objectCustomLabel)
                    exporter.values(str(objectCustomLabel), objectResponse)
            else:
