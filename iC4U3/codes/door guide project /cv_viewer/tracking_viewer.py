...
def render_2D(left_display, img_scale, objects, is_tracking_on):
    ...
    for obj in objects.object_list:
        if render_object(obj, is_tracking_on):
            ...
            global textHandPos, textKnobPos
            if (objectCustomLabel == 'hand'):
                textHandPos = text_position
            elif objectCustomLabel == 'knob':
                textKnobPos = text_position
            else:
                textHandPos = (0,0)
                textKnobPos = (0,0)

            if ((objectCustomLabel == 'hand') and (obj.tracking_state == sl.OBJECT_TRACKING_STATE.OK)):
                lineColor = (255,0,0)
                lineThickness = 2
